...
use bevy_pmetra::{
    cad_core::extensions::{face::FaceCadExtension, transform::UvFaceTransform},
...
use super::{CadMaterialIds, RoundCabinSegment};

pub fn build_cabin_shell(builder: &CadShellsBuilder<RoundCabinSegment>) -> Result<CadShell> {
...
pub fn build_cabin_mesh(
    ...
    let cad_mesh = CadMeshBuilder::new(builder.params.clone(), cad_shell.clone())? // builder
        ...
        .add_cursor(
            CabinCursorIds::ExtrudeCursor.to_string(),
            build_extrude_cursor,
        )?
        .add_cursor(
            CabinCursorIds::CornerRadiusCursor.to_string(),
            build_corner_radius_cursor,
        )?
        .add_cursor(
            CabinCursorIds::ProfileThicknessCursor.to_string(),
            build_profile_thickness_cursor,
        )?
        .add_cursor(
            CabinCursorIds::ProfileHeightCursor.to_string(),
            build_profile_height_cursor,
        )?
        .add_cursor(
            CabinCursorIds::ProfileWidthCursor.to_string(),
            build_profile_width_cursor,
        )?
        .add_cursor(
            CabinCursorIds::WindowTranslationCursor.to_string(),
