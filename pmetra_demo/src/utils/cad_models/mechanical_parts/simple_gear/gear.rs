...
use bevy_pmetra::{
    cad_core::extensions::{shell::ShellCadExtension, wire::WireCadExtension},
...
/// - `B`   : tooth thickness.
///
pub fn build_main_gear_shell(builder: &CadShellsBuilder<SimpleGear>) -> Result<CadShell> {
    let SimpleGear {
        num_of_teeth,
...
pub fn build_main_gear_mesh(
    ...
    let cad_mesh = CadMeshBuilder::new(builder.params.clone(), cad_shell.clone())? // builder
        ...
        .add_cursor(
            GearCursorIds::PitchCircleDiaCursor.to_string(),
            build_radius_cursor,
        )?
        .add_cursor(
            GearCursorIds::FaceWidthCursor.to_string(),
            build_face_width_cursor,
        )?
        // .add_cursor(
        //     GearCursorIds::NumTeethCursor.to_string(),
        //     build_num_teeth_cursor,
        // )?
        .build()?;
...
pub fn build_radius_cursor(
    builder: &CadMeshBuilder<SimpleGear>,
    cad_shell: &CadShell,
) -> Result<CadCursor> {
    let SimpleGear {
        num_of_teeth,
...
mod tests {
    ...
    #[test]
    pub fn test_solid() {
        let builder = CadShellsBuilder::<SimpleGear>::default();
        build_main_gear_shell(&builder).unwrap();
    }
