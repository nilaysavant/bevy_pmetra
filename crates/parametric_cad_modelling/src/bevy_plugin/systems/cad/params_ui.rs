...
use crate::{
    ...
    cad_core::builders::{CadCursorName, CadMeshName, ParametricCad},
...
pub fn show_params_display_ui_on_hover_cursor<Params: ParametricCad + Component>(
    ...
    for CursorPointerMoveEvent { target, hit } in events.read() {
        let Ok((
            cursor_name,
            BelongsToCadGeneratedMesh(cad_mesh_ent),
            BelongsToCadGeneratedRoot(cad_root_ent),
        )) = cursors.get(*target)
        else {
            continue;
        };
        ...
        let Ok(mesh_name) = generated_meshes.get(*cad_mesh_ent) else {
            continue;
        };
        // Update UI text...
        let Ok(tooltip) = params.on_cursor_tooltip(mesh_name.clone(), cursor_name.clone()) else {
            continue;
        };
